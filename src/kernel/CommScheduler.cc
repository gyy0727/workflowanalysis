...
//*根据传入的 abstime 参数决定是调用 pthread_cond_timedwait 还是
//*pthread_cond_wait。
//*带超时的等待：如果 abstime 不为空（即为有效的 struct timespec 指针）
//*则调用 pthread_cond_timedwait。这意味着线程将在条件变量变为真之前等待
//*但等待时间不会超过 abstime 所指定的时间。
//*不带超时的等待：如果 abstime 为空，则调用 pthread_cond_wait。
//*这意味着线程将在条件变量变为真之前无限期等待，除非有其他线程改变了条件变量的状态。
